...
parser.add_argument("-l", "--log_name", help="server log name", type=str, required=True) # \CartPoleSwingUpFixInitState-v1_ppo\
parser.add_argument("-n", "--custom_name", help="custom names for each log", type=str)
...
if __name__ == "__main__":
    ...
    custom_name = args.custom_name if args.custom_name else args.log_name.split('\\')[-1]
    ...
    for prefix in prefixes:
        try:
            ...
            for metric, (x, y) in metrics_data.items():
                ...
                all_metrics_data[metric].append((prefix, x, y))
    ...
    # print(f"All plots for prefix {prefix} have been saved to {os.path.join(args.save_dir, custom_name)}")
